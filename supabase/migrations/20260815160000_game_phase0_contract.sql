-- Formal post-P-1 NingAcademy Game P0 contract. This new migration is derived
-- from, but does not reactivate or mutate, the frozen pre-P-1 audit draft.
-- Scheme B unlock requirements are added by the immediately-following
-- 20260815170000 migration so the archived draft remains byte-identical.
--
...
-- Supabase owns auth.uid() and does not give the migration executor grant
-- options on the auth schema/function. Keep game_api_owner out of auth and
-- expose only the current UUID through this postgres-owned narrow wrapper.
create function private.current_game_user_id()
returns uuid
language sql
stable
security definer
...
revoke execute on function private.current_game_user_id()
...
grant usage on schema private to game_api_owner;
grant execute on function private.current_game_user_id() to game_api_owner;
...
begin
  ...
  where t.id = private.current_game_user_id();
...
begin
  ...
  -- request ids, so only one unused launch ticket survives. An advisory lock
  -- avoids giving the SECURITY DEFINER owner UPDATE on public.profiles merely
  -- to obtain a row lock.
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(v_user_id::text, 47616)
  );
...
begin
  ...
  v_caller_id := private.current_game_user_id();
  v_teacher_id := private.current_teacher_id();
