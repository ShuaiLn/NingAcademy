...
import { assignPronunciationTaskToTargets, unassignPronunciationTarget } from "@/app/actions/pronunciation";

type ClassOption = { id: string; name: string };
type StudentOption = { id: string; fullName: string };
type TargetRow = { id: string; classId: string | null; studentId: string | null };

// Mirrors app/teacher/assignments/[id]/assign-panel.tsx -- see its comment
// for why this uses router.refresh() rather than optimistic client state.
export function AssignPanel({
  ...
  function handleAssign() {
    ...
    startTransition(async () => {
      const result = await assignPronunciationTaskToTargets(taskId, selectedClasses, selectedStudents);
      if (result.ok) {
        setSelectedClasses([]);
        setSelectedStudents([]);
        router.refresh();
  ...
  function handleUnassign(targetId: string) {
    setError(null);
    startTransition(async () => {
      const result = await unassignPronunciationTarget(taskId, targetId);
      if (result.ok) {
        router.refresh();
      } else {
        setError(result.error);
