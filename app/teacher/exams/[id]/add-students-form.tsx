...
import { addStudentsToExam } from "@/app/actions/exams";
import { getClassRoster } from "@/app/actions/classes";

type ClassOption = { id: string; name: string };
type StudentOption = { id: string; fullName: string };

// Post-creation variant of app/teacher/exams/new/target-picker.tsx -- same
// "class checkbox is a one-time roster-resolution convenience, flat student
// list is authoritative" behavior, but scoped to students not already on
// this exam (already-added ids are filtered out of both the class-resolved
// roster and the individual list, since add_students_to_exam() would just
// no-op on them anyway and showing them again invites confusion).
export function AddStudentsForm({
  ...
  function handleSubmit() {
    setError(null);
    startTransition(async () => {
      const result = await addStudentsToExam(examId, checkedStudentIds, usedClassIds);
      if (result.ok) {
        setUsedClassIds([]);
