...
The audit used isolated branch `codex/p1-audit-20260825-final`, commit
`ab7cdc192b26aa96c34b19ae7cb607a2b508f1d5`, and workflow dispatch input
...
Migration 28 made `game_api_owner` the schema/object owner and left `postgres`
with neither `USAGE` nor `CREATE` on `game` after its temporary membership is
removed. Supabase CLI 2.114.0's canonical local reset connects as `postgres`,
uses one session, wraps each migration file transactionally and runs `RESET ALL`
before every file. Migration 30 explicitly `SET ROLE game_api_owner`, returns
with `SET ROLE postgres`, then revokes the temporary membership. The job did
not emit a separate identity query, but the pinned executor source plus that SQL
timeline deterministically establish that at statement 0 of migration 31,
`session_user`, `current_user` and the effective role are all `postgres`. This
is the migration/DDL identity, not `games_api` or the future restricted Games
LOGIN. The replay harness therefore matches the canonical execution contract.

Unlike migrations that use the established temporary membership plus `SET ROLE
game_api_owner` pattern, migration 31 begins directly with `create function
game.*`, then would attempt `ALTER FUNCTION ... OWNER TO game_api_owner` without
membership. It is intrinsically defective under the intended contract. The
same review found migration 33 begins with another direct `create function
game.*`, while migration 34 later alters/creates owner-only `game` and
`game_private` objects, again without acquiring/entering `game_api_owner`.
Migration 32 and migration 35 contain the explicit choreography. The replay
history and exact clean prefix end at migration 30. A new migration 35 cannot
execute before 31 and therefore cannot close a from-zero replay while 31–34
remain immutable. No workflow pregrant, skip, repair, or hidden state was
introduced to disguise the failure, and no rerun was launched after confirming
that only an explicitly approved history amendment can change the result.
...
and Git-vs-replay histories fail closed because migrations 31–35 are undeclared
and unapplied. The replay snapshot is the exact post-30 prefix,
...
catalog gate, but run `32926070717` recorded it as skipped with status 125 because
replay cannot reach migration 35. Migration 35's SQL/static contract is checked;
its resulting PostgreSQL catalog is not yet validated.
...
next decision is an explicit owner/governance authorization for a reviewed,
pre-deployment amendment of the frozen executor defects in migrations 31, 33
and 34 (including their expected hashes and inventory), or another explicitly
approved policy-compliant history resolution. Only then can a 35/35 replay and
fresh protected audit be treated as release evidence.
...
## Historical P-1 PRE-DEPLOYMENT READY decision (superseded by final run 32926070717)
